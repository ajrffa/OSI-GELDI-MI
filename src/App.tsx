...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[linear-gradient(90deg,#ff8904_50%,#C0392B_50%,#C0392B)]">
      ...
      <div
        ref={iframeRef}
        className="fixed top-0 left-0 bottom-0 p-4 z-10 w-full flex justify-center"
      >
        <div className="w-full max-w-[600px] aspect-video border-4 border-[#82181a] rounded-2xl overflow-hidden shadow-lg bg-black">
          <iframe
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 relative z-20">
        <div className="flex flex-col items-center space-y-4 sm:space-y-6">
          <button
            ref={buttonRef}
            onClick={handleClick}
            className="bg-gradient-to-r from-orange-500 to-yellow-500 text-red-700 px-6 sm:px-10 md:px-12 py-4 sm:py-5 md:py-6 text-2xl sm:text-4xl md:text-5xl font-extrabold rounded-2xl shadow-2xl border-4 border-red-700 hover:scale-110 transition-transform duration-300 text-center"
          >
            OSIMHEN GELDİ Mİ?
          </button>
          ...
          {message && (
            <div
              key={clickCount}
              className="mt-4 w-full max-w-md p-4 sm:p-6 bg-black bg-opacity-10 backdrop-blur-lg rounded-xl shadow-xl text-center text-white animate-fade-in transition-all duration-500"
            >
